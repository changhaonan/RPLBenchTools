...
class AxisBBox3D:
    ...
    def create_axis_aligned_from_points(self, points, axis_points=None):
        ...
        if axis_points is None:
            ...
            # Determine the major direction; x, y or z
            axis_dir = np.array(axis_points[1] - axis_points[0])
            axis_dir = axis_dir / (np.linalg.norm(axis_dir) + eps)
            max_axis_dir = np.argmax(np.abs(axis_dir))
            main_axis_dir = np.zeros(3)
            main_axis_dir[max_axis_dir] = 1
            axis_points = np.array(axis_points)
            if axis_dir[max_axis_dir] > 0:
                axis_points[0] = (
                    min_bound[max_axis_dir] - axis_points[0, max_axis_dir]
                ) / (axis_dir[max_axis_dir] + 1e-6) * axis_dir + axis_points[0]
                axis_points[1] = (
                    max_bound[max_axis_dir] - axis_points[1, max_axis_dir]
                ) / (axis_dir[max_axis_dir] + 1e-6) * axis_dir + axis_points[1]
            else:
                # Pointing from 1 to 0
                axis_points[0] = (
                    max_bound[max_axis_dir] - axis_points[0, max_axis_dir]
                ) / (axis_dir[max_axis_dir] + 1e-6) * axis_dir + axis_points[0]
                axis_points[1] = (
                    min_bound[max_axis_dir] - axis_points[1, max_axis_dir]
                ) / (axis_dir[max_axis_dir] + 1e-6) * axis_dir + axis_points[1]
            self.axis = axis_points
    ...
    def select_pcd(self, pcd):
        """Select the point cloud within the bounding box."""
        local_pcd = pcd - self.center
        local_pcd = np.dot(local_pcd, self.R)
        mask = np.all(np.abs(local_pcd) < self.extent / 2, axis=1)
